# -*- coding: utf-8 -*-
"""
Created on Sun Feb  3 12:16:17 2019
...
def leastInterval(tasks, n):
    ...
    wait_T = collections.Counter()
    n_tasks = collections.Counter()
    ...
    wait=0
    counter=0
    candidate = [ta[0] for ta in n_tasks.most_common(len(tasks))]
    for i in range(len(tasks)):
        j=0
        while wait_T[task]!=0 and j<len(candidate):
            if n_tasks[task]!=0: task = candidate[j]
            j+1
        ...
        counter=0
        for k in wait_T.keys():               
            if wait_T[k]>=wait+1: wait_T[k]-=(wait+1)
            elif wait_T[k]<wait: wait_T[k]=0                
        wait_T[task] = n
